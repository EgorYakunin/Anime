import { Document, Filter, MongoClient, OptionalId, UpdateFilter } from 'mongodb';

class Mongo {
    static client = new MongoClient('mongodb://localhost:27017/anime');
    ...
    static async write_one(collection_name: string, object: OptionalId<Document>) {
        try {
     ...
     * Not available yet
     */
    static async write_many(collection_name: string, object: OptionalId<Document>[]) {
        try {
            await this.start_connection();
            const collection = this.client.db().collection(collection_name.toString());
    ...
    static async update_one(collection_name: string, filter: Filter<Document>, object: Partial<Document> | UpdateFilter<Document>) {
        try {
            await this.start_connection();
            const collection = this.client.db().collection(collection_name.toString());
    ...
    static async delete_one(collection_name: string, filter: Filter<Document>) {
        try {
            await this.start_connection();
            const collection = this.client.db().collection(collection_name.toString());
